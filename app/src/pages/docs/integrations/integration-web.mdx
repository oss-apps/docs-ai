...
<Tabs items={['HTML', 'Angular', 'React', 'NextJs']} >
  ...
  <Callout type="warning">
    The values should be set whenever the page loads and before user clicks the chat bot icon.
  </Callout>
  ...
  <script>
    window.DOCS_AI = {
      ...
      additionalFields: {
        userEmail: "hey@docsai.app",
        name: "Shrihari",
        companyId: "c_id_124xyt6yqlgs"
      }
  }
  </script>
  ```
  ...
  <Tabs.Tab>
    ...
    function App() {
      return (
        <>
          <Helmet>
            <script src="https://docsai.app/embed.min.js" project-id="<your_project_id>" async>
            </script>
          </Helmet>
        </>
        )
