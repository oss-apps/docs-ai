...
Currently Websites and Plain Text can be to the DocsAI. We are working on adding support for PDFs, Notion etc.
Send us an email at `hey@docsai.app` if you want to request a specific document type.

###### Where is the document stored?
We don't store the document directly and store the embeddings of the document in pinecone database. It's currently hosted in US region
...
Send an email to `hey@docsai.app` or DM me on [Twitter](https://twitter.com/KM_Koushik_)